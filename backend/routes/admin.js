...
router.post("/login", loginLimiter, async (req, res, next) => {
  try {
    ...
    res.json({ admin: { id: admin.id, username: admin.username, email: admin.email } });
  } catch (err) {
    next(err);
  }
});
...
router.post("/forgot-password", loginLimiter, async (req, res, next) => {
  try {
    ...
    const admin = email ? await findAdminByEmail(email.toLowerCase().trim()) : null;
...
router.post("/reset-password/:token", async (req, res, next) => {
  try {
    ...
    const admin = await findAdminByValidResetToken(tokenHash);
    if (!admin) {
      return res.status(400).json({ message: "This reset link is invalid or has expired." });
    }
    const passwordHash = await bcrypt.hash(password, 12);
    await updatePassword(admin.id, passwordHash);
