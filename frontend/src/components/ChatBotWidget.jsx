...
function getBotReply(input) {
  ...
  if (/(ai|chatbot|automat)/.test(text)) {
  ...
  if (/(market|seo|ads|campaign|social|brand)/.test(text)) {
...
export default function ChatBotWidget() {
  ...
  const handleSend = (e) => {
    ...
    const delay = 500 + Math.min(reply.length * 8, 900);
